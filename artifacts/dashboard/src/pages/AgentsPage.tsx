...
import { api, Agent, TrainingItem } from '@/lib/api';
import { Plus, Bot, Pencil, Trash2, ToggleLeft, ToggleRight, X, Loader2, ChevronDown, BookOpen, ChevronRight } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3 scrollbar-thin">
          ...
          {items.map(item => (
            <div key={item.id} className="flex gap-3 p-3 bg-muted/50 rounded-xl border border-border group">
              <p className="flex-1 text-sm whitespace-pre-wrap break-words leading-relaxed">{item.content}</p>
              <button onClick={() => del.mutate(item.id)}
                className="p-1 rounded hover:bg-red-100 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity shrink-0 self-start">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <textarea value={form.system_prompt} onChange={e => setField('system_prompt', e.target.value)}
              rows={5} placeholder="Você é um assistente de atendimento da empresa X. Responda sempre em português, de forma educada e objetiva."
              className="w-full px-3 py-2 rounded-lg border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" />
          </div>
          <div>
